...
int	ainstr(t_asm *a, char toks[][64], int n)
{
	...
	if (!strcmp(toks[0], "and") && n == 3 && preg(toks[1], &r1, &s1))
	{
		...
		else if (s1 == 32)
		{
			val = sym(a, toks[2]);
			if (val < 0) val = strtoll(toks[2], NULL, 0);
			emit_and_r32_imm32(&a->out->e, r1, (uint32_t)val);   /* REVERT : toujours forme longue */
		}
		return 0;
	}
	if (!strcmp(toks[0], "neg") && n == 2 && preg(toks[1], &r1, &s1))
	{
		if (s1 == 8)  { emit_neg_r8(&a->out->e, r1); return 0; }
		if (s1 == 32) { emit_neg_r32(&a->out->e, r1); return 0; }
		if (s1 == 64) { emit_neg_r64(&a->out->e, r1); return 0; }
	}
	if (!strcmp(toks[0], "add") && n == 3 && preg(toks[1], &r1, &s1) && r1 == REG_RSP)
    { emit_add_rsp_imm32(&a->out->e, (uint32_t)strtoll(toks[2], NULL, 0)); return 0; }
	if (!strcmp(toks[0], "add") && n == 3 && preg(toks[1], &r1, &s1) && s1 == 64)
	{
		val = sym(a, toks[2]);
		if (val < 0) val = strtoll(toks[2], NULL, 0);
		if (val >= -128 && val <= 127)
			emit_add_r64_imm8(&a->out->e, r1, (int8_t)val);
		else
			emit_add_r64_imm32(&a->out->e, r1, (int32_t)val);
		return 0;
	}
	if (!strcmp(toks[0], "add") && n == 3 && preg(toks[1], &r1, &s1) && s1 == 32)
	{
		val = sym(a, toks[2]);
		if (val < 0) val = strtoll(toks[2], NULL, 0);
		if (val >= -128 && val <= 127)
			emit_add_r32_imm8(&a->out->e, r1, (int8_t)val);
		else
			emit_add_r32_imm32(&a->out->e, r1, (uint32_t)val);
		return 0;
	}
			...
			** variante possible tient dans remaining, sinon fallback NOP ── */
			{
				...
				if (choice == 0 && remaining >= 7)      /* _INC r32 : jusqu'a 7 octets (REX+81+modrm+imm32) */
				{
				...
				else if (choice == 2 && remaining >= 4) /* _ZERO r32 : jusqu'a 4 octets (corrige de 6 a 4) */
				{
        ...
        const t_opts *opts, t_format_backend *backend)
{
    ...
    ** ne pourrait pas retrouver la cle depuis le stub genere ── */
	if (opts->use_lde)
    {
		...
		fprintf(stderr, "[DEBUG] buf[0..10] reel = ");
		for (int k = 0; k < 10; k++) fprintf(stderr, "%02x ", a.out->e.buf[ss+k]);
		fprintf(stderr, "\n");
...

		{
			...
			{
				int bit = lde_bit_at_pos((size_t)p);
				if (bit == -1) continue;

				int is_expected = 0;
				for (int i = 0; i < g_bit_log_len; i++)
				{
					if (g_bit_log_off[i] == (size_t)p) { is_expected = 1; break; }
				}
				if (is_expected)
					expected_count++;
				else
